...
<Note>New self-service requests require a confirmed TOTP or registered passkey; see [Profile & security](/en/guides/profile) for enrollment steps and retry. Open links are re-served before this gate; API-key automation remains exempt.</Note>
<Steps>
...
- **Third-party banking**: `POST /v1/banking/third-parties` requires the
  ...
  response). Details in
  [Banking](/en/guides/banking#third-party-banking-users-companies-only).
- **Cards for designated persons**: `POST /v1/cards` with a person
  ...
  **approved KYC**. The cardholder's identity and documents come from the
  ...
  `salary_usd`). Details in [Cards](/en/guides/cards).
...
[AML screening](/en/guides/aml) checks sanctions/PEP/adverse-media lists and
can watch the identity continuously.
</Accordion>
<Accordion title="Can I reuse a customer's verification in other products?">
...
<Accordion title="Do I get an email when my verification is approved or rejected?">
