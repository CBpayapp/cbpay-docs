...
`bank_transfer`) are unchanged: see [payouts](/en/guides/payouts).
</Note>
...
[the money model](/en/concepts/money-model#choose-which-balance-receives-your-payins).
...
[BOB virtual accounts guide](/en/guides/bob-virtual-accounts). A person
...
- **Settlement delay**: when your `payin_card` fee is configured with
  ...
  applies when a [checkout link](/en/guides/checkout) or a
  [POS charge](/en/guides/qr-pos) is paid by card. Details in
...
- `save_card: true` plus `payer_reference` store the card with the payer's
  ...
  [stored cards and subscriptions](/en/guides/stored-cards-subscriptions)).
...
<Tab title="International SWIFT (BIC)">
...
<Card title="Checkout" icon="cart-shopping" href="/en/guides/checkout" horizontal>
  One link where the payer chooses how to pay - fiat in every live country,
  crypto, card or the CBPay app - settled in the balance you choose.
</Card>
...
<Card title="Stored cards & subscriptions" icon="credit-card" href="/en/guides/stored-cards-subscriptions" horizontal>
  Save cards with the payer's consent, charge them one-click or without the
  payer present, and schedule recurring subscriptions.
</Card>
...
<Card title="Payin refunds" icon="rotate-left" href="/en/guides/refunds" horizontal>
  Refund a card payin fully or partially, void a same-day charge, and
  understand how a chargeback is applied.
</Card>
...
<AccordionGroup>
<Accordion title="How do I know a payin was credited?">
...
<Accordion title="Which FX rate applies to my payin?">
...
<Accordion title="Can payins land in a balance other than USDT?">
...
<Accordion title="What happens when a charge (QR, checkout) expires unpaid?">
You receive `payin_expired` and the payin closes without moving money.
Create a new charge — nothing was debited or credited.
</Accordion>
<Accordion title="The payer transferred a different amount than announced — what now?">
...
<Accordion title="Two clients announced the same amount and neither sent the reference — who gets the money?">
...
<Accordion title="Do I have to send payer_document now?">
...
<Accordion title="I retried the announcement POST — did I create two announcements?">
...
<Accordion title="How does my US client pay — domestic wire or international SWIFT?">
...
<Accordion title="Why is the bank QR just text to copy, not something my banking app scans?">
