...
one — see [Billing address: state/region required by country](/en/guides/payins#billing-address-stateregion-required-by-country)).
...
<Steps>
<Step title="The payer types their email">
...
<Step title="They verify it with the code (once per device)">
...
<Step title="They pick the card and pay">
...
<Accordion title="Why does the payment page ask for a code sent to the payer's email?">
...
<Accordion title="Must the payer verify their email on every payment?">
