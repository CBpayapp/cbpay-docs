...
| `wallet_import` | Fijo por importación | Al importar una wallet externa a una [wallet segregada](/es/guias/wallets-segregadas) (`POST /v1/segregated-wallets/import`) |
...
| `kyc_verification` | Fijo por verificación | Al crear un link o submission KYC de un tercero ([verificación](/es/guias/kyc)); tu propio onboarding es gratis |
| `kyb_verification` | Fijo por verificación | Al crear un link o submission KYB de un tercero |
| `address_screening` | Fijo por scan | Al evaluar el riesgo de una dirección blockchain ([screening de wallets](/es/guias/screenings)); la protección automática de retiros/depósitos es gratis |
...
| `risk_report_person` | Fijo por informe | Al comprar un informe crediticio Qscore de una persona ([Qscore](/es/guias/qscore)); se reembolsa automáticamente si el informe falla (`risk_report_refund`) |
...
`method: "card"`, un [link de checkout](/es/guias/checkout) pagado con
tarjeta y un [cobro POS](/es/guias/qr-pos) pagado con tarjeta comparten la
