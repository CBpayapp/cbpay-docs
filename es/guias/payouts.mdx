...
comprobante PDF, el export CSV de payouts y la cartola. Las respuestas, CSV, webhooks y comprobantes también incluyen `uetr` (UETR/SWIFT gpi) e `imad` (wire/ACH doméstico), vacíos hasta que el banco los reporte; usa el [tracker público](/es/guias/seguimiento) para el estado vivo y el mismo comprobante.
</Note>
...
Para conocer el origen de la tasa y las cotizaciones FIFO indicativas, revisa [Orígenes de tasa de payouts y cotizaciones FIFO](/es/guias/payout-rate-sources).
...
Consulta la [guía dedicada de payouts SEPA Instant](/es/guias/sepa-instant-payouts)
...
<Step title="Pásalo al crear el payout">
...
<Card title="Payout QR" icon="qrcode" href="/es/guias/qr-payout" horizontal>
...
<AccordionGroup>
<Accordion title="¿Cuándo se debita mi saldo?">
...
<Accordion title="¿Puedo cancelar un payout en processing?">
...
<Accordion title="¿Qué tasa FX usa mi payout?">
...
<Accordion title="¿Puedo pagar desde un saldo distinto de USDT?">
...
<Accordion title="¿Qué significa `status: pending` con `compliance_pending: true`?">
...
<Accordion title="¿Qué significa compliance_hold (403)?">
...
<Accordion title="¿Cómo reintento sin riesgo tras un timeout o 5xx?">
...
<Accordion title="Por que mi payout US ACH/wire/SWIFT sigue processing?">
