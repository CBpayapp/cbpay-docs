...
slug: en/guides/disputes
lang: en
source_url: https://docs.cbpayapp.com/en/guides/disputes
---
...
When an organization generates an evidence pack, the account can download the
same authenticated, private PDF associated with the case. It contains typed
payin fields, ledger/FIFO evidence and external spend references.

For card payins, the pack includes the processor-facing card operation
(transaction references, approval, amount/currency/date, masked card details,
issuer/country, 3DS, AVS/CVV, decision-manager result, device fingerprint and
masked payer context). FIFO spends include the withdrawal or payout ID,
asset/currency, status, destination address or bank reference and transaction
hash when available. The merchant's verified KYB/KYC identity is included in
full for processor representation; the payer remains masked. Raw payin
payloads and PAN-adjacent fields are excluded.
...
See the [error catalog](https://docs.cbpayapp.com/en/errors) for the common response envelope.

## FAQ

#### Can I release or forfeit a case from the account API?
No. Decisions are organization-admin actions protected by `disputes:write` and
maker-checker rules.
#### Does an expired case release the hold?
