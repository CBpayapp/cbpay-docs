...
This event is emitted when an inbound EUR payment addressed to a virtual IBAN
has been normalized for the owning account. It includes `iban`, `amount`,
`currency`, `status`, `reference`, `payment_rail` (`SEPA_INSTANT`) and the
stable `provider_event_id`. Deduplicate by the delivery event ID and use the
same idempotent handling as other money events.

```json
{
  "event_type": "banking_virtual_iban_inbound",
