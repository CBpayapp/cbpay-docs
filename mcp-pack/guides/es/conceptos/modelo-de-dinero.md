...
---
## Seis saldos virtuales independientes

Cada cuenta mantiene **seis saldos virtuales, uno por moneda**. Son
...
`GET /v1/balances` devuelve siempre los seis (con ceros si no has operado
...
wallets, banking) pueden debitarse desde cualquiera de tus seis saldos.
...
`SILVER` y `PLATINUM` son saldos ledger-only respaldados por custodio y medidos en gramos finos con seis decimales. Settlement y swaps usan un oráculo de precios y la conversión fija de `31.1034768` gramos por onza troy; esto no implica un riel on-chain de depósito, retiro o gasto de metales.
