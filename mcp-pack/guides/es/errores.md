...
| `invalid_purpose` | `purpose` debe ser `fondeo` o `banking`; elige un corredor Banking soportado |
...
| `invalid_cuit` | Los payouts de Argentina (`AR`) exigen que `beneficiary.tax_id` sea un CUIT o CUIL de 11 dígitos — envía el identificador de 11 dígitos del beneficiario; consulta la [guía de payouts](https://docs.cbpayapp.com/es/guias/payouts) |
| `invalid_cbu` | Los payouts de Argentina (`AR`) exigen que `beneficiary.account_number` sea un CBU o CVU de 22 dígitos; los payouts en USD aceptan solo CBU — envía un CBU/CVU válido de 22 dígitos (CBU para USD); consulta la [guía de payouts](https://docs.cbpayapp.com/es/guias/payouts) |
...
| `invalid_locale` | `PATCH /v1/me` (o el registro) envió un locale no vacío fuera de `en` / `es` / `zh`. El string vacío guarda inglés. Distinto de `invalid_language` (`lang` de informe PDF) y del `invalid_value` de org-admin en `default_locale` |
...
| 422 | `issuer_data_incomplete` | Faltan o son inválidos los campos del emisor después de ensamblar el request, el autofill de verificación y los datos KYB; corrige las claves indicadas en `message` y reintenta |
| 422 | `screening_required` | La cuenta no tiene un screening AML de verificación para derivar el PEP del titular; completa el screening y reintenta el mismo request de tarjeta |
| 422 | `cardholder_name_too_long` | `first_name` y `last_name` de la persona superan juntos el límite de 22 caracteres impresos por el emisor; acorta el nombre y reintenta |
...
| 422 | `unsupported_country` | Este flujo de fraude/identidad de Qscore no está disponible para el país seleccionado — usa un país compatible o contacta a soporte |
| 503 | `recovery_pending` | El informe de fraude todavía se está finalizando — consulta el informe con las mismas credenciales y reintenta el PDF cuando llegue a `ready` |
...
| HTTP | `error` | Significado |
|---|---|---|
| 400 | `unsupported_chain` | La `chain` debe ser `tron` o `eth` |
| 400 | `invalid_statement` | El `statement` supera los 280 caracteres — acórtalo (ojo: el sobre firmado acepta máximo 140) |
| 409 | `proof_not_signable` | El comprobante ya no está en un estado firmable (ya firmado, expirado o cancelado) — solicita uno nuevo |
| 409 | `custody_transferred` | La custodia de la wallet fue transferida al cliente, así que la firma server-side ya no es posible — firma con tu propia clave |
| 409 | `challenge_consumed` | Este desafío ya fue usado o ya no es válido — solicita uno nuevo |
| 410 | `challenge_expired` | Este desafío expiró — solicita uno nuevo y fírmalo dentro de su ventana de validez |
| 422 | `signature_mismatch` | La firma no corresponde a la dirección del desafío — firma el mensaje exacto del desafío con la wallet que estás vinculando |
| 422 | `sign_rejected` | El firmante rechazó la solicitud — el `message` trae la razón saneada. Un `statement` de 141–280 caracteres también cae aquí (el sobre acepta máximo 140) |
...
## Errores de recuperación de creaciones banking

| Código | HTTP | Significado y acción |
|---|---:|---|
| `claim_identity_mismatch` | 422 | La identidad del cliente del proveedor no coincide con la solicitud banking original. Detén la recuperación y revisa el claim. |
...
| 409 | `payin_not_credited` | El payin no está `credited`, por lo que no hay saldo que retener; espera la acreditación antes de abrir un caso o vincular una fila importada. |
...
### Errores de límites money-out

| HTTP | `error` | Significado |
|---|---|---|
| 422 | `amount_below_minimum` | El monto está bajo el mínimo efectivo; consulta `GET /v1/org/txn-firewall/limits/effective`. |
| 422 | `amount_above_maximum` | El monto supera el máximo efectivo; revisa la política antes de reintentar. |
| 422 | `velocity_limit_exceeded` | Una transferencia interna superó un tope móvil de monto o cantidad. Payouts y retiros entran a review con `reason=velocity_limit`. |
| 503 | `limits_unavailable` | No se pudieron evaluar los límites antes del débito; reintenta con la misma clave de idempotencia. |
