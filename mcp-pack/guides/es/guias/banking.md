...
## Operaciones EUR SEPA y el vIBAN de origen

Para `currency: "EUR"` y un `WITHDRAW` con `paymentType: "SEPA_CT"`, la
operación debe resolver un IBAN virtual asignado y activo con propósito
`banking_eur`. La fuente se selecciona en el nivel superior:
...
```json
{
  ...
  "type": "WITHDRAW",
  "paymentType": "SEPA_CT",
  "source_virtual_iban_id": "<UUID_VIBAN_BANKING_EUR_ACTIVO>"
}
```

Si la cuenta tiene exactamente un vIBAN `banking_eur` activo, puedes omitir el
campo. Sin una fuente activa responde `422 funding_account_required`; con varias
fuentes activas sin UUID explícito responde `422 ambiguous_source_viban`.
Un `source_virtual_iban_id` con formato inválido responde `400
invalid_request`; un UUID válido que no existe o no pertenece a la cuenta
responde `404 not_found`. Un fallo temporal de lectura responde
`503 funding_account_unavailable`.

La plataforma elimina los campos de identidad ordenante enviados por el caller
y estampa `payer` server-side desde el `registrant` del vIBAN seleccionado,
salvo que exista un payer de wallet corporativa más específico y también
server-side. El registrant persona aporta nombre y apellido; el corporativo,
razón social registrada y datos de registro.

En filas activas legacy sin registrant utilizable, la plataforma deriva los
nombres desde el perfil verificado vigente. Si no puede leer ese perfil,
devuelve `503 funding_account_unavailable`; si la identidad queda incompleta,
devuelve `422 registrant_incomplete`.

No existe un tope por vIBAN. EUR Banking descuenta del saldo `BANK_EUR` de la
cuenta; `funding_usdt` es un propósito separado del flujo de fondeo de payouts.

> **Nota**
Los holds de operaciones Banking creados antes de esta puerta quedan
grandfathered y continúan por su flujo existente de dispatch y reconciliación.
## Idempotencia y casos límite del vIBAN de origen
...
## Casos límite de idempotencia y vIBAN de origen
