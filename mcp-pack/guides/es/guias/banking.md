...
## Operaciones SEPA EUR y el IBAN virtual de origen

Para `currency: "EUR"` y una operación `WITHDRAW` con
`paymentType: "SEPA_CT"`, la operación debe financiarse desde un IBAN virtual
asignado y activo cuyo propósito sea `banking_eur`. El origen se selecciona en
el nivel superior del request:
...
Si la cuenta tiene exactamente un IBAN virtual `banking_eur` activo, el campo
se puede omitir. Sin una cuenta activa se devuelve
`422 funding_account_required`; con varias cuentas activas y sin UUID
explícito, `422 ambiguous_source_viban`. Una falla temporal al consultar el
origen devuelve `503 funding_account_unavailable`.

La plataforma ignora los campos de identidad ordenante enviados por el caller
y estampa `payer` desde el `registrant` del IBAN virtual seleccionado. El
registrant de una persona aporta nombre y apellido; el de una empresa aporta
la razón social y sus datos de registro. No existe un tope por IBAN virtual:
la operación debita el saldo `BANK_EUR` de la cuenta.

> **Nota**
Los holds de operaciones Banking creados antes de esta puerta quedan
grandfathered y continúan por su despacho/conciliación existente.
