...
- **Vigencia del link para tarjeta**: para materializar una tarjeta, el link
  ...
  comercio un link nuevo antes de pagar con tarjeta.
...
Para una opción C2P materializada de `VE/VES`, el pagador entrega la
`claveDinamica` generada por su banco en el campo `otp` de
`POST {checkout_url}/collect`. No llames a `/collect/otp` para C2P; ese
endpoint solo aplica a un método que emita un OTP antes del cobro.
