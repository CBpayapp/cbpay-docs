...
slug: es/guias/devoluciones
lang: es
source_url: https://docs.cbpayapp.com/es/guias/devoluciones
---
...
|---|---|---|
| `pending` | La pedimos al procesador y todavía no hay respuesta definitiva. Tu saldo ya está reservado. Mientras siga pendiente, la respuesta incluye `reconciliation_required: true`. | Esperar el webhook. **No** reintentes con otra clave: duplicarías la devolución. |
...
cuando queda en curso (con `reconciliation_required: true`) y `422` cuando
rechaza. Un resultado ambiguo queda persistido en el mismo claim de
devolución y se reconcilia por el webhook del procesador o el sweep de solo
lectura; la plataforma nunca lo vuelve a despachar.
...
| 503 | `refund_event_pending` | El resultado quedó guardado, pero todavía se persiste su evento final. Reintenta con la **misma** clave de idempotencia; no envíes otra devolución. |
