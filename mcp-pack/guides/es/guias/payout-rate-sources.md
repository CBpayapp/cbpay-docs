...
`payin_rate` es la cotización del lado de depósitos. En USDT v1 puede estar
respaldada por lotes payin habilitados, fondeados por trades `buy_crypto`
liquidados; `payin_rate_source` informa `lot` o `spot`. Create/detail de payin
puede incluir `rate_source` cuando cargó una cotización. Los payins con tarjeta
siguen usando spot. El endpoint para administradores de organización expone la
misma distinción con `payout_rate`, `payin_rate`, `rate_source` y
`payin_rate_source`.
...
evento final `payout_status_changed` son la fuente autoritativa. Para payins,
la cotización queda fijada antes del crédito asíncrono; el consumo FIFO registra
la atribución al acreditar. Un faltante cae a spot y genera una alerta operativa
sin cambiar el crédito cotizado.
