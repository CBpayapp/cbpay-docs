...
`payout_status_changed` puede llevar `status: "pending"`, `status_code:
"compliance_pending"`, `compliance_pending: true` y `funds_debited: false`.
No incluye `receipt_url` porque todavía no existe débito ni comprobante.
