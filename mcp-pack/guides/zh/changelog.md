...
- 新增按账户保留 BOB、MXN、ARS 本地法币收款，并返回实际资产字段。
...
- 组织现在可以由平台 operator 启用新的 BOB 内部 QR 收款 route。公开
  `POST /v1/payins` request、opaque `charge` response 和现有
  `payin_received` webhook 保持不变；可用性按组织控制，集成方不能
  选择内部 route。
...
- 修正 SEPA 来源门控：客户 payout 要求已分配且 active 的 `funding_usdt`
  虚拟 IBAN，EUR Banking 提现使用 `banking_eur`。
- 文档补充确定性来源选择、从 vIBAN registrant 服务端写入付款人身份、
  legacy profile fallback，以及 `funding_account_unavailable` /
  `registrant_incomplete` 结果。
### v2.120

**变更**

- payin 汇率文档现在公开 `payin_rate_source`（`lot` 或 `spot`）以及
  payin create/detail 响应中可选的 `rate_source`。USDT FIFO payin 库存
  在入账时消费；库存不足时回退到 spot，不改变已经报价的入账金额。
...
- 客户账户的 SEPA Instant payout 现在必须使用已分配且 active 的
  `funding_usdt` 虚拟 IBAN。存在多个 active EUR 资金账户时使用可选字段
...
- **门户的专属收款优先流程**：在 `/payins/new` 中，玻利维亚（`BO`）和
  墨西哥（`MX`）现在始终显示账户的专属收款账户，与“我的账户”一致。
  如果没有专属收款工具，门户会显示诚实的空状态，并引导用户创建账户或
  完成身份验证；这些走廊不再在门户中提供预告转账的付款人选项。API 路由和
  响应结构不变，集成方仍可使用预告转账的 `POST /v1/payins`。
### v2.117
...
- QR payout 指南现在记录同步两步合同：不扣款 scan、幂等 confirm、
  可选 `settlement_asset`，以及状态为临时状态时通过已有 payout 进行对账。
...
- 组织管理员现在可以从粘贴的 payin 标识符列表预览并确认打开争议。
  流程会先解析唯一匹配，在冻结前暴露歧义，只打开已 credited payin，
  并返回逐项幂等结果。
...
- **我的账户 F2**：`/accounts` 页面现在使用视觉选择器：Crypto（资产 →
  网络）、Fiat（国家 → 转账资料卡）和 Banking（国家 → 账户）。美国
  Banking 账户按 ACH、Wire 和 SWIFT 子选项卡分组。
- 账户 hero 在有数据时显示持有人照片/姓名、alias 和账户 QR。Crypto 与
  Banking 资料卡可以显示带组织品牌标识的 QR；Fiat 资料卡按设计不显示
  QR。
- Fiat 转账资料卡在 API 返回时显示持有人、银行、BO 商户 NIT 和收款号码，
  支持逐字段复制与**复制全部**。选项卡和目的地可通过 `?tab=&sel=` 深链
  接入。

**API**

- 存款账户响应新增可选字段 `alias`、`merchant_nit` 和 `bank_name`。
  这些字段只有在收款通道提供时才会出现；客户端不应推断缺失值。
...
- Payout 响应、`payout_status_changed`、CSV 和回执现在会在银行报告后提供
  新增的 `uetr` 与 `imad` tracking 字段。
- Payout 指南说明这些标识与银行确认及公共交易 tracker 的关系。
...
- **我的账户**：门户现在提供统一的 `/accounts` 页面，集中显示账户
  QR 与 alias、本地收款目的地、USD Banking 账户、EUR 虚拟 IBAN 与企业
  wallet，以及加密货币充值钱包。每个分区保留现有的 QR、复制和分享
  操作，并显示诚实的空状态或处理中状态。

**变更**

- 门户中的 **我的钱包** 标签现在显示为 **Wallets**。钱包 ID、地址、
  资产和只接收行为不变。
- MX/CLABE 与 BO/BOB 收款目的地只有在账户 KYC/KYB 审核通过后才会
  配置。现有 API 路由和响应结构不变。
...
- 通过 `BO/BOB/bank_transfer` 支持 BOB 收款账户。

**变更**

- 个人账户每个走廊保留一个收款目的地；企业账户可使用不同的 `idempotency_key` 创建额外目的地，重复同一密钥会幂等重放。
- 只有账户的 KYC/KYB 审核通过后才会配置收款账户。面向提供商的别名由服务器根据已验证姓名生成，收款账户路由与 payout 路由保持分离。

### v2.99

**变更**

- 账户交易审核上传现在接受可选的固定 `category` query 参数；此前
  request-info 后，只要审核仍为 `in_review`，也可以继续上传证据。每次
  上传都会显示在审核中并重置 maker-checker 的第一次批准；未知类别返回
  `400 invalid_category`。当 portal base 可用时，request-info 邮件会直接
  链接到 `/reviews/{reviewID}`。

### v2.98

**新增**
- 已验证企业的 EUR 钱包：持久化预留、账户与 org-admin 读取、实时余额和日期操作。
- Core 钱包预留/状态/wallet/statement 路由，以及欧洲 `EU`/`EUR`/`sepa` OTC catalog。

**变更**
- Banking EUR 虚拟 IBAN 余额现在返回 `received_total`，不再伪造每个 IBAN 的 `available`；企业可创建多个虚拟 IBAN，个人账户仍按 purpose 限制。
- 组织费用接受 EUR，treasury 显示运营 EUR 钱包 source。

**错误**
- 覆盖 `wallet_not_ready`、`wallet_reservation_failed`、`wallet_reservation_status_failed`、`wallet_read_failed`、`wallet_statement_failed` 和 `virtual_iban_limit_reached`。

### v2.97

**变更**

- **金属 settlement 防护加固**：XPT/PLATINUM 执行参考价在可用时会使用
  第二个独立的公开参考价。偏离超过 2% 时，该报价不再具备 settlement
  资格；二级参考不可用时，现有参考价新鲜度 guard 仍然生效。现有 API
  路径与 response shape 不变。
- **更真实的汇率新鲜度**：由参考源支持的汇率现在使用来源 payload 的
  timestamp；超过七天默认容忍期的 payload 会在发布前被拒绝。
- **金属卡默认值**：SILVER 的默认缓冲和降级 haircut 为 3%；BTC 仍为
  5%，GOLD 为 2%，PLATINUM 为 2%。组织级 override 仍可用。
...
- **银行卡 checkout 的最短有效期**：银行卡物化现在接受剩余至少 5 分钟的
  checkout 链接。剩余时间不足时返回 `422 checkout_expiring_soon`，并提示
  付款前向商户索取新链接，而不是稍后以通用的银行卡会话校验错误失败。
...
- **Payout 和 payin 列表筛选器**：payout 列表现在支持 `method` 和
  `search`；payin 列表现在支持 `method`、`kind`、`refund_status` 和
  `search`。
...
- **公开状态页遵循通道生命周期**：运营方通过管理控制停用的通道不再出现在状态列表或事故历史中，也不再影响整体状态。已启用但处于中断状态的通道仍会显示。
...
- **认证账户创建 payin 需要已批准的身份**：个人账户必须通过 KYC，企业账户必须
  通过 KYB，之后才能创建 payin 或使用账户级 collect。`POST /v1/payins`
  （QR、card、checkout、bank_transfer 和 fintoc）、collect OTP/collect、
  创建专属入金账户、已保存卡扣款和订阅在获批前均返回
  `403 verification_required`。已经存在的公开支付链接、已经收到的被动入账、
  org-admin 运营分配以及系统创建的入金工具仍可用。
...
- **成员安全事件**：账户范围的 webhook 传送会 redact `ip` 和
  `user_agent`；组织范围的管理员视图保留取证用的 `user_agent` 以及
  actor provenance 字段。
- **Webhook V2 签名文档**：webhook 指南现在说明了
  `X-Webhook-Signature-V2`。该签名使用 HMAC-SHA256 签署
  `timestamp.event_type.event_id.raw_body`，并与传统 V1 的
  `timestamp.raw_body` 签名同时发送。V1 仍然有效。
...
- **Payout 技术筛查队列**：当收款人筛查暂时不可用但队列可用时，
  `POST /v1/payouts` 返回 `202`，资源为 `status: pending`、
  `status_code: compliance_pending`、`compliance_pending: true` 与
  `funds_debited: false`。等待期间不会创建 hold、账本分录、core 派发或
  `receipt_url`。相同幂等键 replay 返回同一个 payout。`process` 继续正常
  流程，有效 `hold` 进入防火墙，`rejected` 在不扣款的情况下将队列中的
  payout 结束为 `failed`。
...
- **Collect 与托管 checkout 的合规暂留**现在在付款人扣款前发生。批准只
  释放审核；使用相同的幂等上下文重试同一个 collect 请求。不明确的
  timeout/5xx 会保持 pending 并等待 webhook 对账，绝不会自动重发。
  新增 `checkout_not_pending` 冲突说明，并记录只有证件的付款人应作为
  独立身份 screening（`document_value`/`idNumber`，而非 `name`）。
- **交易筛查现在区分已识别的 AML 类别与模糊候选结果**：已识别的类别在
  `target=true` 或 `target=false` 时仍可触发行动。当 `target=false` 且 AML
  类别未被识别时，结果不可触发行动，也不会产生错误的暂留或错误响应。
  缺少 `target` 的旧版响应，以及不完整或格式错误的响应，仍然采用
  fail-closed 行为。
...
- **欺诈与身份报告明确声明征信覆盖**：没有征信证据时，报告仍可能以
  `THIN_FILE` 就绪，包括明确的无数据源情况。PDF 现在声明来源新鲜度
  （`live` 或 `unavailable`）和记录数量。传输、认证、存储、契约无效以及
  非空 `sources_failed` 仍通过现有 recovery/退款路径 fail-closed。

### v2.70

**新增**

- **最终银行恢复语义**：记录 `400 idempotency_key_mismatch`、不会终结 claim 的提供商 `401`/`403`、基于 `ProviderID` 的所有权验证、无需再次调用提供商的完成重放、稳定恢复锁，以及公共/平台 body key 不转发。

- **欺诈与身份报告**：新增 `POST/GET /v1/qscore/fraud/reports`、认证 detail/PDF、四个 purpose、幂等计费与失败退款、签名 `risk_fraud_score_ready` webhook、best-effort 邮件和 `/verify/qscore-fraud/{code}` 公开验证。
- **Qscore Index**：新增 `/v1/qscore/index` 和 `/v1/qscore/index/account` 的季度快照、国家/行业筛选以及至少五个主体的匿名桶。
...
- **以卡支付的收银台和 POS 收款现在遵循银行卡结算延迟**。当您的
  `payin_card` 费用配置了大于零的 `settlement_hours` 时，以**银行卡**
  支付的托管收银台链接或 POS 收款与直接银行卡收款行为完全一致：立即
  确认为 `credited` 并关闭为已支付，但**余额**到 `settle_at` 才到账
  （在此之前携带 `settlement_pending: true`，可由机构管理员提前释放）。
  此前这些收款计收银行卡费用但却立即入账。通过 QR、加密货币、CBPay
  应用或 fintoc 支付的收款仍立即入账。API 和 webhook 的结构均未变化。
  详见[费用——银行卡收款结算延迟](https://docs.cbpayapp.com/zh/concepts/fees#银行卡收款结算延迟)。
