...
收款寻址和路由工具，并不是供应商的独立余额。账户 API 暴露持久化申请和状态。下文记录已验证的入金行为；provider onboarding 和 returns 仍是单独的 acceptance gate。
...
> **重要**
生产 onboarding（主机、证书/mTLS、限额、商业费用和启用日期）尚未成为
公开合同。不要从 blueprint 示例推断这些信息。
### 错误
...
> **注**
Returns/recalls、provider statement 对账以及生产 fees/onboarding 合同仍是
单独的 acceptance work，不能从此事件推断。
## EUR 入金可追溯性
