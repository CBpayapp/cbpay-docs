...
当 `currency: "EUR"` 且 `WITHDRAW` 使用 `paymentType: "SEPA_CT"` 时，
操作必须从一个已分配且 active、purpose 为 `banking_eur` 的虚拟 IBAN
中扣款。来源字段位于请求顶层：
...
没有 active 账户时返回 `422 funding_account_required`；有多个 active
账户但未提供明确 UUID 时返回 `422 ambiguous_source_viban`。资金来源
检查暂时不可用时返回 `503 funding_account_unavailable`。

平台忽略调用方提交的付款人身份字段，并从所选虚拟 IBAN 的 `registrant`
服务端写入 `payer`。个人 registrant 提供名和姓；企业 registrant 提供
注册公司名称及注册信息。没有按虚拟 IBAN 设置的金额上限；操作从账户的
`BANK_EUR` 余额扣款。

> **注**
在此门控上线前创建的 Banking 操作 hold 属于 grandfathered 记录：继续
使用原有 dispatch/reconciliation 流程。
## 幂等与来源 vIBAN 边界情况
