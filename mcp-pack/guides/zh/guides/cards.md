...
- 卡面印刷姓名使用 `first_name` + `last_name`（合计最多 22 个字符），响应中包含 `cardholder_kind: "person"` 以及所使用的 `verification_id`。如果合计超过卡面打印限制，API 返回 `422 cardholder_name_too_long`。
...
| 持卡人 | 创建时必填 | 值如何解析 |
|---|---|---|
| 个人账户持卡人 | `occupation`、`place_of_work`、`salary_usd`、`first_name`、`last_name`、`email` | `occupation` 必须是目录代码；`place_of_work` 不能为空；`salary_usd` 必须非负；姓名不能为空且 `email` 必须包含 `@` |
...
| 指定人员 | `occupation`、`place_of_work`、`salary_usd`、`first_name`、`last_name`、`email`、`pep` | 目录代码、非空姓名、含 `@` 的 email 和显式布尔值 `pep`；指定人员没有账户 screening |

对于账户持卡人，受支持的 PEP 来源是账户 AML 身份验证的 screening。
...
#### 如果账户还没有 AML screening，会怎样？
系统无法派生持卡人的 PEP。完成已批准的 AML 身份验证后，用同一个卡片
...
| 422 | `screening_required` | 账户没有可用于派生持卡人 PEP 的 AML 身份验证 screening。完成 screening 后用同一个 request 重试。 |
...
> **注**
个人还是企业？两种账户类型在所有产品中的差异汇总见[个人与企业](https://docs.cbpayapp.com/zh/concepts/persons-companies)。
### 职业与经营活动（目录代码）
