...
当发送到虚拟 IBAN 的 EUR 入金被规范化到 owner account 时发送。负载包含
`iban`、`amount`、`currency`、`status`、`reference`、`payment_rail`
（`SEPA_INSTANT`）和稳定的 `provider_event_id`。按 delivery event ID 去重，
并使用其他资金事件相同的幂等处理。
