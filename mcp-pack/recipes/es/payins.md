...
> **Importante**
**Se requiere identidad aprobada antes de cobrar con la cuenta autenticada.**
Una cuenta persona necesita KYC aprobado y una cuenta empresa necesita KYB
aprobado antes de crear un payin. Aplica a `POST /v1/payins` (QR, `card`,
`checkout`, `bank_transfer` y `fintoc`), `POST /v1/payins/collect/otp`,
`POST /v1/payins/collect` y `POST /v1/payins/deposit-accounts`, además de
cobros con tarjeta guardada y `POST /v1/subscriptions`. Sin aprobación, estas
requests responden HTTP `403 verification_required`.
> **Nota**
Este gate aplica a la creación autenticada de la cuenta. Las páginas públicas
...
> **Nota**
**Venezuela (VES)**: el cobro activo está disponible mediante `c2p`. El
pagador genera una `claveDinamica` en la app de su banco y te la entrega; tu
...
fija de payin. Los lotes payin se consumen FIFO al aplicar el crédito. Si el
inventario habilitado no alcanza, la cotización existente sigue siendo
...
gratis; cada depósito paga la comisión de payin normal. Los campos opcionales
...
`MX`/`MXN`/`bank_transfer` (CLABE) y `BO`/`BOB`/`bank_transfer` (cuenta
...
no crea una cuenta de depósito. El alias que recibe el proveedor se genera en
el servidor a partir del nombre verificado de la cuenta.

Para crear una CLABE de empresa, envía una clave de idempotencia en el
body o en el header:
...
Una clave nueva responde `201` y crea un destino nuevo para el corredor
seleccionado. Repetir la
misma clave responde `200` con el instrumento original y
`idempotency_hit: true`; una repetición concurrente mientras sigue
en vuelo puede responder `409 idempotency_conflict`. Crear es gratis;
los payins posteriores conservan la comisión normal. El destino BOB es solo
receptor y se concilia por polling; no crea ni selecciona una cuenta de origen
para el ruteo de payouts. Lista los
destinos con `GET /v1/payins/deposit-accounts?page=1&page_size=50`.
...
- `customer` es un prefill **opcional** de los datos de facturación
  ...
  código ISO 3166-2 (`US-CA`) o su sufijo (`CA`).

**Dirección de facturación — estado/región requerido según el país**: la
página hosted recolecta la **dirección de facturación completa** (nombre,
correo, calle, país, ciudad, estado/región y código postal). El campo
**estado/región** es **obligatorio siempre que el país de facturación tenga
subdivisiones ISO 3166-2** (por ejemplo Estados Unidos, Canadá, México,
Brasil o Chile): el pagador lo elige de una lista desplegable alimentada por
el catálogo de direcciones, y la página no envía el pago sin él. Si el país
no tiene subdivisiones en el catálogo, el campo se oculta. El valor viaja
como `administrative_area` (ISO 3166-2). La dirección de facturación se
valida **antes** de enviar el cargo a autorización: si está incompleta, o
falta el estado/región en un país que lo exige, el pago se rechaza con
`invalid_payload` y **no se crea ninguna autorización sobre la tarjeta**.

> **Nota**
Una autorización creada con una dirección de facturación incompleta puede
ser rechazada después al momento de la captura por el emisor. Recolecta
siempre la dirección de facturación completa — incluido el estado/región
...
Algunos corredores ofrecen un cobro pull donde el pagador autoriza un débito.
La cuenta debe tener su identidad aprobada antes del request.

### C2P de Venezuela

Para `VE/VES/c2p`, el pagador primero genera una `claveDinamica` en la app de
su banco. Tu integración envía ese valor como `otp`; no es un OTP que CBPay
devuelva y no existe un paso de solicitud de OTP:
...
El request usa el contrato collect existente. `payer_document`, `payer_phone`,
`payer_bank` y `otp` son los datos que necesita el riel C2P; `otp` lleva la
clave dinámica generada por el banco. La respuesta conserva el shape actual y,
si se aprueba en forma síncrona, incluye `paid: true`, `provider_reference` y
los identificadores del payin acreditado.

### Métodos pull basados en OTP

```bash
`POST /v1/payins/collect/otp` se usa solo cuando el método pull activo emite un
OTP antes del cobro. No forma parte del flujo C2P. Si lo llamas para C2P, la
API responde el `422 provider_rejected` público; no lo interpretes como un
cobro C2P fallido ni reemplaces C2P por `debito_inmediato`.
```

La plataforma evalúa al pagador antes de llamar al core. Un hold responde
`202 in_review` sin cobrar; reintenta el mismo request después de la
aprobación. Si la cuenta no está aprobada, el collect responde
`403 verification_required` antes de ejecutar el corredor. Un timeout o 5xx
no autoriza una operación nueva: conserva el mismo contexto de idempotencia y
sigue el estado de la operación. Revisa siempre
`GET /v1/payins/methods`, porque la disponibilidad puede cambiar.
...
exactamente a esa tasa: `usdt_gross = 700.00 / 6.91`. Cuando aparece,
`rate_source` indica el origen de la cotización (`lot` o `spot`); los payins
históricos sin cotización cargada omiten el campo.
...
# Historial con filtros: fechas, estado, país, método, tipo, devolución y búsqueda
...
`400 invalid_range`. `method` y `kind` acotan el riel y el tipo de payin,
`refund_status` acepta `none`, `partial` o `full`, y `search` hace una coincidencia
parcial sin distinguir mayúsculas/minúsculas sobre el ID del payin, la referencia
o la clave de idempotencia.
...
El `payin_rate` cotizado por `GET /v1/rates` y vigente al momento de
acreditar. Revisa `payin_rate_source`: `lot` significa que el inventario USDT
