...
| `risk_report_person` | 每份报告固定费用 | 购买个人 Qscore 信用报告时([Qscore](/zh/guides/qscore));报告失败时自动退款(`risk_report_refund`) |
...
以卡支付的[收银台链接](/zh/guides/checkout)以及以卡支付的
[POS 收款](/zh/guides/qr-pos)都遵循相同的 `settle_at` 规则（包括自身
