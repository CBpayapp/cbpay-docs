...
| 每个网络+资产组合的**充值钱包**（[加密货币](/zh/guides/crypto)） | **1 个**（随账户创建；仅用于接收） | **1 个**（随账户创建；仅用于接收） |
| **隔离钱包**（[独立链上余额](/zh/guides/segregated-wallets)） | **每个网络+资产组合 1 个** | **不限**（用 `label` 加以区分） |
...
- 卡片：**不限数量** — 企业卡（持卡人 = 企业本身，首张卡需企业
  ...
  [指南](/zh/guides/cards)。
- 成员：添加拥有独立登录和权限的用户
  （`owner`/`operator`/`viewer`）— [指南](/zh/authentication#company-members)。
