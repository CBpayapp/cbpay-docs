...
[BOB 虚拟账户指南](/zh/guides/bob-virtual-accounts)。个人账户每个走廊
...
- **结算延迟**：当 `payin_card` 费用配置了大于零的 `settlement_hours`
  ...
  入账金额。以卡支付[收银台链接](/zh/guides/checkout)或
  [POS 收款](/zh/guides/qr-pos)时适用相同的延迟。详见
  [费用——银行卡收款结算延迟](/zh/concepts/fees#银行卡收款结算延迟)。
...
- `save_card: true` 配合 `payer_reference` 会在获得付款人同意后保存卡片
  以便后续收款（参见
  [已保存卡片与订阅](/zh/guides/stored-cards-subscriptions)）。
...
<Card title="Checkout 收款链接" icon="cart-shopping" href="/zh/guides/checkout" horizontal>
...
<Card title="已保存卡片与订阅" icon="credit-card" href="/zh/guides/stored-cards-subscriptions" horizontal>
...
<Card title="退款" icon="rotate-left" href="/zh/guides/refunds" horizontal>
...
<Accordion title="我的美国客户如何付款 —— 境内电汇还是国际 SWIFT？">
...
<Accordion title="为什么银行 QR 只是一段供复制的文本，而不是我的银行 App 能扫描识别的内容？">
