...
将你的端点订阅到 `corridor_status_changed` 事件（[webhook 指南](/zh/webhooks)），每次状态转换发生时你都会立即收到——中断**与**恢复都会推送：
